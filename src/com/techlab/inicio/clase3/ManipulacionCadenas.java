...
public class ManipulacionCadenas {
  ...
  public static String formatear(String cadena) {
    ...
    for (int i = 0; i < palabras.length; i++) { //Se ejecuta según la cantidad de palabras
      if (!palabras[i].isEmpty()) { //Verifica que la Palabra no esté vacía
        sb.append(Character.toUpperCase(
                palabras[i].charAt(0))) //Convierte el primer carácter en mayúscula
    ...
    return sb.toString()
        .trim(); //se borra el espacio final y todas las palabras se concatenan retornando un solo string.
  }
